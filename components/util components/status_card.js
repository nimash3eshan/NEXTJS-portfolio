  ...
  componentDidMount() {
    ...
    if ("getBattery" in navigator) {
      navigator.getBattery().then((battery) => {
        // Update all battery info once at the start
      ...
      console.log("Browser doesn't support the Battery Status API.");
    }
  }
        ...
        <div className="w-64 py-1.5 flex items-center justify-center bg-ub-cool-grey hover:bg-ub-warm-grey hover:bg-opacity-20">
          <div className="w-8">
            <img
              width="20px"
              height="20px"
							//check this.state.battery_charging. if true rotate 270deg else rotate 0deg
							style={this.state.battery_charging ? { transform: "rotate(270deg)" } : { transform: "rotate(0deg)" }}
              src={
                this.state.battery_charging
                  ? "./themes/Yaru/status/battery-charging-symbolic.png"
          ...
          <div className="w-2/3 flex items-center justify-between text-gray-400">
            <span>{`${this.state.battery_level}% (${
              this.state.battery_charging ? "Charging" : "Not charging"
            })`}</span>
            <SmallArrow angle="right" />
          </div>
        </div>
